...
#scp ~/Downloads/license_rh_org.lic   azure@bastion.4psh8.azure.opentlc.com:/tmp  
...
oc create secret docker-registry rtf-pull-secret --namespace rtf --docker-server=rtf-runtime-registry.kprod.msap.io --docker-username="$dockerUsername" --docker-password="$dockerPassword"
 ...
 base64 -i /tmp/license_rh_org.lic > output.txt

yq e -i  '.metadata.namespace = "'$projectname'"' installing-rtf-openshift/configs/runtimefabric.yaml
yq e -i  '.spec.activationData = "'$activationdata'"' installing-rtf-openshift/configs/runtimefabric.yaml
yq e -i  '.spec.muleLicense = "'$(cat output.txt)'"'  installing-rtf-openshift/configs/runtimefabric.yaml
oc apply -f installing-rtf-openshift/configs/runtimefabric.yaml
echo "installing-rtf-openshift/configs/runtimefabric.yaml" > .gitignore